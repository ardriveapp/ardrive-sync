...
export const getFileUploadTimeFromSyncTable = (id: number) => {
...
export const getAllUploadedDataItemsFromDriveTable = (login: string, bundleTxId: string) => {
	return all(`SELECT * FROM Drive WHERE login = ? AND bundleTxId = ?`, [login, bundleTxId]);
};

export const getAllUploadedFilesFromSyncTable = (login: string) => {
	return all(
		`SELECT * FROM Sync WHERE (login = ?) AND (bundleTxId IS NULL) AND (fileDataSyncStatus = 2 OR fileMetaDataSyncStatus = 2)`,
		[login]
	);
};
